...
- **注册表 v2.1 完成待用户终审**(feat/t3-registry,未合并):按用户裁决——主表 393 公开 / 受限表 19 parked(公务员 12+国企 7,拆 registry/edges/aliases-restricted.csv 三件套,永不进 seed、绝不上服务器、未来特殊权限+AI 拒答);AI 新兴扩容 4(FDE/具身智能/GPU集群/AI评测,带长期需求论证);MCN 转正;TOP12-30 十项补判;edges 主 838/受限 47 两份零悬空过。
- **⚠️ 隔离缺口(上线前必修)**:司法系统族 5 条政法编制岗漏隔离仍在主表(法院执行员/法官助理/检察官助理/监狱警察[省考]/公证员)——按用户"考公一律不公开"应移 restricted;律师 2 条(诉讼/非诉)是市场职业正确留主表。待用户确认边界一并移(连带 edges)。
- **🛑 用户 2026-07-09「做完这一层停」**:发展层+v2.1 两层收口完毕,停;不启动回归 5 条/经济批/量产。待用户:注册表 v2.1 终审(评审稿开放问题 Q2 剩 36 条 L3 + 司法编制 5 条隔离确认)→批准后走回归 5 条→经济批 go/no-go。
