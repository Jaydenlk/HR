...
- **后端 Jest e2e**:正常(有/无 JD)+ 异常(简历过短、职业无法识别、预设缺失)+ 防编造(简历无某经历 → 确认不虚构)。
- **AI 复杂场景**:同一简历换不同预设 → 评分/建议**确有职业倾向差异**;低质简历 → 拒绝/低置信。
- **前端 Playwright E2E**:桌面 + 移动,逐按钮逐流程真人走查:选职业 →(选填 JD)→ 发起 → 看结果各区块 → 改写示范交互。
