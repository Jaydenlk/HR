...
const RESUME = JSON.stringify({
  ...
  skills: { technical: ['SQL', 'Axure'], soft: [], languages: [], certifications: [] }, projects: [],
});
...
describe('AnalyzerService.analyzeAgainstPreset', () => {
  ...
  it('rejects too-short resume', async () => {
    await expect(svc.analyzeAgainstPreset('短', productManagerCampus)).rejects.toBeInstanceOf(BadRequestException);
  });
});

(process.env.CLOUDDREAM_API_KEY ? describe : describe.skip)(
  'AnalyzerService.analyzeAgainstPreset (AI live)',
  () => {
    let svc: AnalyzerService;
    ...
    it('returns preset dimensions with non-empty why each', async () => {
