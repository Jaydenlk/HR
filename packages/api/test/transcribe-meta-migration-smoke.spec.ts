/**
 * AddTranscribeTaskUploadMeta 迁移结构冒烟(不依赖运行中的 Postgres,捕获迁移发出的 SQL 做结构断言)。
 *  - up() 发出 4 条对 interview_transcribe_tasks 的 ADD COLUMN(original_filename varchar /
 *    file_size_bytes integer / mime_type varchar / uploaded_at TIMESTAMP),全可空。
 *  - 纯加法:不 CREATE/DROP TABLE,不 ALTER 任何其它表(只动 interview_transcribe_tasks)。
 *  - down() 逆序回滚:4 条 DROP COLUMN,顺序与 up 的 ADD 相反(uploaded_at 先,original_filename 最后)。
...
import { AddTranscribeTaskUploadMeta1781600000000 } from '../src/database/migrations/1781600000000-AddTranscribeTaskUploadMeta';
...
const ADDED_COLUMNS: ReadonlyArray<readonly [string, RegExp]> = [
  ['original_filename', /"original_filename" character varying/],
  ['file_size_bytes', /"file_size_bytes" integer/],
  ['mime_type', /"mime_type" character varying/],
  ['uploaded_at', /"uploaded_at" TIMESTAMP/],
];

describe('AddTranscribeTaskUploadMeta1781600000000 迁移结构', () => {
  const migration = new AddTranscribeTaskUploadMeta1781600000000();

  it('类可实例化且 name 与文件时间戳一致', () => {
    expect(migration.name).toBe('AddTranscribeTaskUploadMeta1781600000000');
  });

  it('up() 对 interview_transcribe_tasks 发出 4 条 ADD COLUMN,列与类型齐全(全可空)', async () => {
  ...
  it('down() 逆序回滚:4 条 DROP COLUMN,顺序与 up 的 ADD 相反', async () => {
    const { runner, sql } = makeRecordingRunner();
  ...
  it('up/down 列对称:up ADD 的每列 down 必 DROP', async () => {
    ...
    for (const [col] of ADDED_COLUMNS) {
      const added = up.sql.some((q) =>
        new RegExp(`ADD COLUMN "${col}"`).test(q),
      );
      const dropped = down.sql.some((q) =>
        new RegExp(`DROP COLUMN "${col}"`).test(q),
      );
