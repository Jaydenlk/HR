...
export const CAREER_EVIDENCE_IRON_LAW = [
  ...
  `给中高分(current≥4)必须在 evidenceFound 里照抄简历中支撑该分的原文片段,绝不可空;evidenceFound 必须是简历里真实出现过的原话,严禁只照抄技能名本身、或编造简历里没有的文字来充当证据。`,
  ...
  // 可疑量化指标铁律(对齐 analyze-profession-standard.ts:22 的诊断侧范式):无来源数字不采信。
  `可疑量化指标铁律:简历里"罕见高值且无基数/无口径/无成本支撑"的数字(如 2 个月增长 200%、转化率 45%、复购 60%),绝不当作高水平证据为其背书;此类无来源量化数字不采信,相关技能不得因它而进高分区间,evidenceFound 不得照抄这类无口径数字充当佐证。`,
  // 相邻领域不得间接推断(对齐诊断侧"直接证据铁律③"):学科背景≠某技能实操水平。
  `相邻领域不得间接推断给分:某技能的 current 必须有该技能本身的直接佐证,严禁用相邻领域或学科背景间接推断给高分——典型反例:数学/统计专业但简历无算法竞赛/无算法项目,算法或数据技能不得给中高分。`,
  // 评分过程不外泄(对齐诊断侧"打分校准①"):用户可见文字只写结论,不写内部计算。
  `评分过程不外泄:evidenceFound 与 paths 的 description 等任何用户可见文字,只陈述事实与结论,严禁把"评分""压分""上限调整""给到满分的 X%""扣 X 分""综合评定给 X 分"等内部计算或分数比例措辞写进去。`,
].join('\n');
