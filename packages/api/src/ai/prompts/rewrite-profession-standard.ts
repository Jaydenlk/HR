...
export function buildRewriteSystem(preset: ProfessionPreset): string {
  return [
    ...
    `背景错配显式声明(高置信才触发):若候选人背景与「${preset.profession}」岗位本质错配(如纯数学/教学背景投技术岗、毫无相关实习/项目/技能且专业方向与岗位无交集),必须在第一条建议(type=gap_advice、original='')顶部显式声明:"背景与该岗位本质错配,建议重审目标方向",并给出可行的过渡路径;不能只靠低分暗示。判断务必保守——只要候选人有任何相关实习/项目/技能/可迁移能力,或属常见可行的跨专业转岗,就不要触发此声明(防误伤跨专业可行者)。`,
    `可疑数字补口径(改进型):若简历已有"罕见高值却无基数/口径"的可疑数字(如增长200%、转化率45%、复购60%),不要放过、也不要替它编造分母;给一条改进型/量化建议,把改法写成"补充基数与口径/计算过程"——例如原句"转化率45%"→ suggested"转化率45%(基数:[具体数字]次曝光中[具体数字]次转化,统计口径:[请填写])",用占位符引导候选人自填真实分母与口径,绝不替填具体数值。`,
    `核心短板进攻性路径(gap_advice):对"无实习 + 项目全是模板/教程复刻"这类核心短板,gap_advice 不要只说"先做再写",要给可差异化的进攻性路径——如对开源项目提交真实 PR/issue 修复、把模板项目改造成自研垂类应用(结合某真实场景做定制功能)、产出可验证的作品集,并说明这样做能在面试里讲出"为什么做、做了什么取舍、结果",reason 标注"需你真实完成后再写入,面试可深挖"。`,
