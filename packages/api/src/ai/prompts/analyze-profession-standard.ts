...
export function buildProfessionStandardSystem(preset: ProfessionPreset): string {
  ...
  return [
    ...
    `打分校准:① 先在心里定分(0~满分整数)再写 why;why、evidenceFound、gap 只陈述最终结论(如"核心能力缺失,仅给基础分"),【严禁】把评分过程或分数比例写进任何用户可见文字——出现"综合评定给予 X 分""上限调整""修正为 X 分""故给满分""满分的 X%""给到满分的约 X%""给分不超过…""扣 X 分"等任何内部计算/比例措辞均属违规。② 满分/高分封顶:只有 gap 基本为空(无实质缺口)的维度才可给满分或接近满分;若 gap 指出任何硬缺口(完全空白/无任何佐证/核心能力缺失/全程只有"参与协助"而无主导),该维度只能落在中低档,绝不能给满分或接近满分。【严禁】"给高分却 why 全是缺口"的自相矛盾。③ "有相关经历但完全无量化数字"的维度只能给低档分(明显低于及格线),完全空白给极低分;此规则对所有维度一致执行,不得选择性放宽。④ standard 为校招友好档但不得无脑高分,扣分须对应简历中真实存在的弱点,不得因"候选人未写出审阅者脑补的底层原理"而随意拔高或贬低。⑤ 底分必须与内容质量挂钩:"严谨性/规范性/细节/格式"这类维度不得仅因"简历看起来工整"就白送满分或接近满分——若简历整体内容薄弱(无实习/项目空泛/全无量化),此类维度同样只能落中低档,绝不能用一个高分维度去拉高整体及格线;给高分必须有简历里真实可见的扎实佐证。`,
