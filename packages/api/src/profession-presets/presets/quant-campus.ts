...
export const quantCampus: ProfessionPreset = {
  ...
  rewriteGuidance: '量化改写侧重:把简历"已有"的策略/回测/竞赛句讲精准(用了什么数学方法、回测绩效指标如何、数据规模多大),用量化精准动词(推导/实现/回测/优化/验证)与可量化指标(夏普比/最大回撤/换手率/信息比/回测周期)增强可信度;只在缺数字处用 [具体数字] 占位;严禁替候选人添加原句没有体现的策略逻辑或绩效数字,凡简历没有的能力一律走"建议补充(gap_advice)"而非编造。',
