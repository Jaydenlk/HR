...
export const testDevCampus: ProfessionPreset = {
  ...
  rewriteGuidance: '测试开发改写侧重:把简历”已有”的框架/用例/缺陷句讲精准(用了什么框架/工具、覆盖规模多大、发现缺陷影响如何),用测试开发精准动词(搭建/封装/设计/覆盖/定位)与可量化指标(用例数/覆盖率/P0缺陷数/日构建次数)增强可信度;只在缺数字处用 [具体数字] 占位;严禁替候选人添加原句没有体现的框架能力或缺陷数字,凡简历没有的能力一律走”建议补充(gap_advice)”而非编造。',
