...
export const algorithmCampus: ProfessionPreset = {
  ...
  rewriteGuidance: '算法改写侧重:把简历”已有”的模型/实验/竞赛句讲精准(用了什么方法、离线指标/在线AB效果如何、数据规模多大),用算法精准动词(训练/微调/部署/蒸馏)与可量化指标(准确率/AUC/CTR提升/推理延迟)增强可信度;只在缺数字处用 [具体数字] 占位;严禁替候选人添加原句没有体现的模型名称/论文/竞赛成绩,凡简历没有的能力一律走”建议补充(gap_advice)”而非编造。',
