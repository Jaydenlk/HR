...
export const frontendCampus: ProfessionPreset = {
  ...
  rewriteGuidance: '前端改写侧重:把简历”已有”的项目/实习句讲精准(用了什么框架/工程化手段、解决什么性能或体验问题、量化指标如何),用前端精准动词(优化/重构/封装/配置)与可量化指标(LCP/FID/CLS/包体积/首屏时间)增强可信度;只在缺数字处用 [具体数字] 占位;严禁替候选人添加原句没有体现的性能数字或技术名词,凡简历没有的能力一律走”建议补充(gap_advice)”而非编造。',
