...
export const dataAnalystCampus: ProfessionPreset = {
  ...
  rewriteGuidance:
    '数据分析改写侧重:把简历"已有"的取数/分析/实验句讲精准(用了什么SQL/Python手段、解决什么业务问题、决策结论如何),用数据分析精准动词(拆解/归因/定义/设计/验证)与可量化指标(指标口径/转化率/留存率/AB实验置信度/数据规模)增强可信度;只在缺数字处用 [具体数字] 占位;严禁替候选人添加原句没有体现的SQL技法或分析结论,凡简历没有的能力一律走"建议补充(gap_advice)"而非编造。',
  resumeConventions:
